...
Previously the validitity of a log level option passed via the CLI `-l` flag was not checked. An invalid option would silently fail and result in logging not working. As of this version, an invalid option will exit the app with an informative error.

Help message output via `-h` has been updated to show valid values.
