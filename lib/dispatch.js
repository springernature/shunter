...
module.exports = function(config) {
	...
	var getErrorContent = function(err, out, req, res, status) {
		return require('./error-pages')(config).getPage(err, out, req, res, status) || api.error(err, status);
	};
	...
	var api = {
		send: function(err, out, req, res, status) {
			...
			if (err) {
				...
				content = getErrorContent(err, out, req, res, status);
			} else if (req.isJson) {
				content = out;
