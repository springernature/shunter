...
module.exports = function(config) {
	...
	var api = {
		send: function(err, out, req, res, status) {
			...
			var getErrorContent = function(err, out, req, res, send) {
				require('./error-pages')(config).getPage(err, out, req, res, function(content) {
			...
			if (err) {
				...
				getErrorContent(err, out, req, res, status, doSend); // getErrorContent will call doSend with the error page content
			} else if (req.isJson) {
