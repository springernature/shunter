	...
	return {
		...
		compileFile: function (fp) {
			...
			if (path.extname(fp) === ext) {
				...
				var templatesIndex = sandboxNS.indexOf(config.structure.templates);
				templatesIndex = templatesIndex < 0 ? 0 : templatesIndex;
				sandboxNS = sandboxNS.slice(templatesIndex);
				splitPath = sandboxNS.split(path.sep);
				if (splitPath.includes(config.structure.templates)) {
