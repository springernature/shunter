		...
		intercept: function(req, res, next) {
			...
			var writeHead = function(code) {
				...
				if (shouldIntercept()) {
					...
					if (req.__proxyTimingFunctionHeadersReceived) {
						statsd.classifiedTiming('proxy_headers_received', req.__proxyTimingFunctionHeadersReceived('Received headers ' + req.url));
					}
				} else {
			...
			var end = function() {
				...
				if (req.__proxyTimingFunctionBodyReceived) {
					statsd.classifiedTiming('proxy_body_received', req.__proxyTimingFunctionBodyReceived('Received body ' + req.url));
				}
