...
/*
			var err;
			// watch for any error from be response
			proxy.on('response', function(incomingMessage) {
				console.log('incomingMessage.statusCode = '+incomingMessage.statusCode);
				if (incomingMessage.statusCode < 400) {
					err = undefined
				} else {
					err = new Error("Backend returned 404");
					err.status = 404;
				}
			});
*/
	...
	proxy.on('proxyRes', function (proxyRes, req, res) {
		...
		if (proxyRes.statusCode == 404) {
			...
			proxyErr = new Error("Backend returned 404");
//			proxyErr =  {
		...
		intercept: function(req, res, next) {
			...
			var writeHead = function(code) {
				...
				if (shouldIntercept()) {
					...
					console.log('--- writeHead ');//: raw Response from the target', JSON.stringify(proxyRes.headers, true, 2));
					config.log.warn('in intercept writeHead, proxyErr= '+proxyErr);
					if (proxyErr) {
							res.write = write;
							res.end = function() {
								res.writeHead = res.__originalWriteHead;
