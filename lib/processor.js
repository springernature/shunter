...
module.exports = function(config, renderer) {
	...
	var processor = {
		...
		proxyFilter: function(req, res, next) {
			proxy.on('proxyRes', function (proxyRes, req, res) {
				console.log('--- proxyRes');//: raw Response from the target', JSON.stringify(proxyRes.headers, true, 2));
				console.log(proxyRes.statusCode);
				console.log(proxyRes.statusMessage);
				console.log(req.originalUrl);
				if (proxyRes.statusCode == 404) {
					var err = new Error("Backend returned 404");
					err.status = 404;
